--비동등결합(NON-EQUI JOIN) : 두개이상의 테이블의 결합조건에 = 연산자가 아닌 다른 연산자를 사용하여
--참(TRUE)안 행을 결합하여 검색
...
--결합조건 : EMP 테이블의 급여(SAL)가 SALGRADE 테이블의 최소급여(LOSAL)부터 최대급여(HISAL) 범위에
--포함되는 행을 결합하여 검색
SELECT EMPNO,ENAME,SAL,GRADE FROM EMP,SALGRADE WHERE SAL BETWEEN LOSAL AND HISAL;

--외부결합(OUTER JOIN) : 결합조건이 참인 행만 결합하여 검색하는 것이 아니라 결합조건이 맞지 않는 행도
--NULL과 결합하여 검색
...
--사원이 없는 부서정보도 검색하기 위해 결합 조건식에서 EMP 테이블에 (+)를 붙여 검색하면 EMP 테이블의
--결합되는 행이 없는 경우 NULL과 결합되어 사원이 없는 부서정보도 검색 - 외부결합
SELECT EMPNO,ENAME,SAL,DNAME,LOC FROM EMP,DEPT WHERE EMP.DEPTNO(+)=DEPT.DEPTNO;
...
--결합조건 : EMP 테이블(WORKER)의 관리자번호(MGR)와 EMP 테이블(MANAGER)의 사원번호(EMPNO)가 같은 행 결합
...
--1999년 채택된 표준 SQL(SQL3)에서는 테이블의 결합조건과 행의 조건을 구분하여 사용될 수 있도록
--다양한 테이블 결합 관련 기능 제공 
...
--NATURAL JOIN : 결합 테이블에 같은 이름의 컬럼이 하나만 있는 경우 같은 이름의 컬럼에 저장된 컬럼값으로 
--같은 행을 결합하여 검색 - 결합조건 생략
--형식)SELECT 검색대상,검색대상,... FROM 테이블명1 NATURAL JOIN 테이블명2

--EMP 테이블과 DEPT 테이블에 저장된 모든 사원의 사원번호,사원이름,급여,부서이름,부서위치 검색
--결합조건 : EMP 테이블의 부서번호(DEPTNO)와 DEPT 테이블의 부서번호(DEPTNO)가 같은 행을 결합
--결합 테이블에 같은 이름의 컬럼값을 이용하여 자동으로 행을 결합하므로 결합조건 생략
...
--JOIN USING : 결합 테이블에 같은 이름의 컬럼이 여러개 있는 경우 같은 이름의 컬럼에 저장된 컬럼값으로 
--같은 행을 결합하여 검색 - 결합조건 생략
...
--INNER JOIN : 결합조건이 참(TRUE)인 행을 결합하여 검색
...
--EMP 테이블과 DEPT 테이블에 저장된 모든 사원의 사원번호,사원이름,급여,부서번호,부서이름,부서위치 검색
--결합 테이블에 같은 이름을 컬럼을 검색할 경우 반드시 테이블을 명확하게 구분하여 표현
...
--EMP 테이블과 DEPT 테이블, SALGRADE 테이블에 저장된 모든 사원의 사원번호,사원이름,급여
--,부서이름,부서위치,등급번호 검색
...
--결합조건이 맞지 않는 행을 NULL과 결합하여 검색 - OUTER 키워드 생략 가능
...
--문제1.부서 테이블과 사원 테이블에서 사번,사원명,부서코드,부서명을 검색하시오.(사원명으로 오름차순 정렬할 것)
...
--문제2.부서 테이블과 사원 테이블에서 사번,사원명,급여,부서명 검색하시오.단,급여가 2000 이상인 사원에
--대하여 사번을 기준으로 오름차순 정렬할 것
SELECT EMPNO,ENAME,SAL,DNAME FROM EMP JOIN DEPT ON EMP.DEPTNO=DEPT.DEPTNO 
    WHERE SAL>=2000 ORDER BY EMPNO;
    
--문제3.부서 테이블과 사원 테이블에서 사번,사원명,업무,급여,부서명을 검색하시오. 단, 업무가 MANAGER 이며 
--급여가 2500 이상인 사원에 대하여 사번을 기준으로 오름차순 정렬할 것 
...
--문제4.사원 테이블과 급여등급 테이블에서 사번,사원명,급여,등급을 검색하시오. 단, 등급은 급여가 하한값과
--상한값 범위에 포함되고 등급이 4이며 급여를 기준으로 내림차순 정렬할 것    
SELECT EMPNO,ENAME,SAL,GRADE FROM EMP JOIN SALGRADE ON SAL BETWEEN LOSAL AND HISAL
...
--문제7.사원 테이블에서 사원명과 해당 사원의 관리자명,해당 사원의 관리자의 관리자명을 검색하시오.
...
--EMP 테이블에서 사원번호가 7521인 사원과 같은 업무를 하는 사원 중 사원번호가 7844인 사원보다 많은
--급여를 받는 사원의 사원번호,사원이름,업무,급여 검색
...
--서브쿼리의 검색결과가 다중행인 경우 > 또는 < 연산자로 컬럼값을 비교하기 위해 서브쿼리 앞에 ANY 
--또는 ALL 키워드를 사용하여 검색
...
--문제4.사원 테이블에서 CLARK와 같은 부서이며, 사번이 7698인 직원의 급여보다 많은 급여를 받는 
